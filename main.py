...
def auth_bdy():
    ...
    try:
        ...
        _log.info(
            f"请使用浏览器打开 {res['verification_url']} 输入授权码 {res['user_code']}")
    ...
    try:
        ...
        for path_conf in Config['SYNC_PATH']:
            try:
                ...
                _log.info(
                    f"开始处理路径 '{path_conf['local']}' | 文件数量 {len(file_list)}"
                )  # 打印文件列表
                # 5.遍历文件列表，将不存在的文件上传
                for file_path in file_list:
                    ...
                    try:
                        ...
                        if file_size >= (FILE_SIZE_LIMITED -
                                         2048):  # 10g减去一部分，避免判断失误
                            _log.warning(
                                f"[{i}] 文件 '{file_path}' 超出10G限制 | 文件大小：{file_size//GB_SIZE}GB"
                            )
                            skip += 1
                            continue
                        ...
                        if file_md5_str == "":
                            ...
                            continue
                        # 数据库中找到了相同md5，代表已上传
                        if FilePath.select().where(
                                FilePath.file_md5 == file_md5_str).first():
                            _log.debug(
                                f"[{i}] 文件 '{file_path}' 已上传 | 文件哈希：{file_md5_str} | 跳过"
                            )
                            skip += 1
                            continue
                        ...
                        for retry_times in range(UPLOAD_RETRY_TIMES):
                            try:
                                fs_id, md5, server_filename, category, rpath, isdir = bdy.finall_upload_file(
                                    ept_file_path, path_conf['remote'])
                                ...
                                if 'pan.baidu.com' not in str(result):
                                    raise result
                                _log.warning(
                                    f"[{i}] 处理文件：{file_path} 遇到网络错误，2s后重试 | {retry_times} | {str(result)}"
                                )
                        ...
                        if not is_upload_success:
                            _log.error(
                                f"[{i}] 处理文件：{file_path} 遇到网络错误，超过重试次数！")
                            raise result  # 依旧抛出异常，跳过这个文件

                        # 4.上传成功，入库
                        cur_file = FilePath(file_path=file_path,
                                            file_name=file_name,
                        ...
                        if "-7" in str(result):
                            _log.error(
                                f"[{i}] 上传失败 '{file_path}' | 错误码-7，远程路径错误或无权访问，请检查本地路径中是否包含特殊字符"
                            )
                        elif "-10" in str(result):
                            _log.error(
                                f"[{i}] 上传失败 '{file_path}' | 错误码-10，云盘空间不足！程序退出..."
                            )
                            time.sleep(1)
                            exit(0)  # 这里其实算正常退出
                        else:
                        ...
                        if file_md5_str != "" and file_name != "":
                            cur_file = ErrFilePath(file_path=file_path,
                                                   file_name=file_name,
                                                   file_md5=file_md5_str)
                            cur_file.save()
                            _log.info(
                                f"[{i}] 错误记录 '{file_path}' 文件哈希：{file_md5_str}"
                            )
                        else:
                            _log.error(f"[{i}] '{file_path}' 严重错误，文件哈希为空！")
                        # 和当前正在处理的文件不同，说明是加密文件
                        if ept_file_path != "" and ept_file_path != file_path and os.path.exists(
                                ept_file_path):
...
if __name__ == "__main__":
    ...
    try:
        ...
        sch.add_job(upload_task,
                    trigger=CronTrigger.from_crontab(SYNC_INTERVAL))
        _log.info(
            f"[start] 根据 cron [{SYNC_INTERVAL}] 启动任务成功，下次运行时间：{gtime.get_next_run_time(SYNC_INTERVAL)}"
        )
        sch.start()  # 启动
    except Exception as result:
        _log.exception(
